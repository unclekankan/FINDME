...
import { getApiKey } from './deepseek.js'
  ...
  _worker = await createWorker('chi_sim+eng', 1, {
    logger: (m) => {
      if (m.status === 'recognizing text') {
        // progress callback — could expose to UI
      }
    },
  })
  return _worker
 ...
 * @returns {Promise<{text: string, progress: number}>}
 */
export async function ocrImage(base64Image, onProgress) {
...
返回严格的 JSON 格式（不要 markdown 代码块）：
{
  ...
  "eraDistribution": "年代分布描述（如：主要集中在2010年代，有少量经典老歌）",
  "tasteLevel": "品味评级（如：广泛且深入 / 偏主流但稳定 / 单一小众深挖型 / 杂食型探索者）",
  "score": 数字(1-10),
  "strengths": ["优点1", "优点2"],
  "weaknesses": ["盲区1", "盲区2"],
...
export async function analyzeText(textContent) {
  const text = await callDeepSeekAPI([
    ...
    {
      role: 'user',
      content: `以下是一份歌单内容，请解析其中所有歌曲并分析歌单主人的音乐品味：\n\n${textContent}`,
    },
  ], { maxTokens: 2500 })
